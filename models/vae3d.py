...
class AddFusion(nn.Module):
    ...
    def forward(self, x, context):
        ...
        out = self.conv(x + context + self.temp_embed.expand(b, -1, -1, h, w).contiguous())
        out = self.norm(out)
        out = nonlinearity(out)
        out = self.dropout(out)
        if self.use_skip:
        ...
        **ignorekwargs
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if ch_fuse[i_level] >= 0:
                ...
                if i_level  <  self.num_resolutions - f_t - fuse_offset:
                    depth = temp_res
...
class VAE3d(nn.Module):
    def __init__(self, 
        ...
        self.disc_loss_scale = self.loss.disc_loss_scale if self.loss is not None else 1.0
        self.mode = mode
